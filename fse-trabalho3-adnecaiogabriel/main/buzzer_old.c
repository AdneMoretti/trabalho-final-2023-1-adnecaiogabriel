...
// // Defina as constantes para outras notas musicais conforme necessário

// void change_state_buzzer(int state) {
//     gpio_set_level(BUZZER_GPIO, state);
// }


// void playTone(float frequency, int duration) {
//     gpio_set_level(BUZZER_GPIO, 1); // Ligar o buzzer
//     vTaskDelay(pdMS_TO_TICKS(duration)); // Aguardar a duração em milissegundos
//     gpio_set_level(BUZZER_GPIO, 0); // Desligar o buzzer
//     vTaskDelay(pdMS_TO_TICKS(100)); // Pausa entre as notas
// }

// void playMelody() {
//     // playTone(261.63, 500); // Toca a nota C4 por 500ms
//     playTone(50, 3000); // Toca a nota D4 por 500ms
//     playTone(500, 3000); // Toca a nota E4 por 500ms
// }
...
void playTone() {
  ...
  if (toneM > 0) { // if this isn't a Rest beat, while the tone has 
    ...
    while (elapsed_time < duration) {
      ...
      vTaskDelay(toneM / portTICK_PERIOD_MS);
